...
interface consProps {
    ...
    listSize:    number
    renderFunction: (index: number) => void
}
...
class ListScroll {
    ...
    domDataCache: DomCache
    observer: observerProps
    renderFunction: (index: number) => void
    constructor(options: consProps) {
        this.opsCheck(options)
    }

    // 传入属性检查
    opsCheck(ops: consProps) {
        const {
            firstItemId,
            lastItemId,
            itemHeight,
    ...
    adjustPaddings(isScrollDown: boolean) {
    ...
    getWindowFirstIndex = (isScrollDown: boolean) => {
        const {
            currentIndex
        } = this.domDataCache
    ...
    topItemCb(entry: EntryProps) {
        const {
            topSentinelPreviousY,
            topSentinelPreviousRatio
        } = this.domDataCache
        console.log(232323, entry)
    ...
    initIntersectionObserver() {
        ...
        const callback = (entries: Array<EntryProps>) => {
